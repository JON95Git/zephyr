...
static void update_event_handler(lv_event_t *event);
...
static void mbox_event_cb(lv_event_t *evt)
{
    if (evt->code == LV_EVENT_DELETE) {
        if (evt->target  == mbox1) {
            mbox1 = NULL;
        }
    } else if (evt->code == LV_EVENT_VALUE_CHANGED) {
        if (evt->target  == mbox1) {
...
static void _create_err_msg_box(const char *error_message)
{
    ...
    mbox1 = lv_msgbox_create(lv_scr_act(), "", error_message, NULL, true);
    // lv_msgbox_set_text(mbox1, error_message);
    // lv_msgbox_add_btns(mbox1, btns);
    lv_obj_set_width(mbox1, 200);
    lv_obj_add_event_cb(mbox1, mbox_event_cb, LV_EVENT_ALL, NULL);
...
static void _gui_create_button(lv_obj_t *parent)
{
    ...
    update_button = lv_btn_create(parent);
    lv_obj_add_event_cb(update_button, update_event_handler, LV_EVENT_ALL, NULL);
    lv_obj_align(update_button, LV_ALIGN_CENTER, 0, 0);

    button_label = lv_label_create(update_button);
