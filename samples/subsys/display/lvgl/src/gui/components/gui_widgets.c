...
{
    lv_obj_t *label;
    label = lv_label_create(label_st->parent);
    lv_label_set_text(label, label_st->text);
    lv_obj_align(label, label_st->align, label_st->x_ofs, label_st->y_ofs);
    return label;
}
...
{
    lv_meter_set_indicator_value(gauge, needle_id, value);
}
