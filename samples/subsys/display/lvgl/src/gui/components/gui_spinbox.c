...
static void lv_spinbox_increment_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_SHORT_CLICKED || code == LV_EVENT_LONG_PRESSED_REPEAT) {
        lv_obj_t *aux = lv_event_get_user_data(e);
...
static void _gui_create_spinbox_plus_button(lv_obj_t *parent, _gui_spinbox_st *spinbox_st)
{
    ...
    spinbox_st->plus_button = lv_btn_create(parent);
    lv_obj_set_size(spinbox_st->plus_button, h, h);
    lv_obj_align_to(spinbox_st->plus_button, spinbox_st->spinbox, LV_ALIGN_OUT_RIGHT_MID, 5, 0);
    lv_obj_set_style_bg_img_src(spinbox_st->plus_button, LV_SYMBOL_PLUS, 0);
    lv_obj_add_event_cb(spinbox_st->plus_button, lv_spinbox_increment_event_cb, LV_EVENT_ALL, spinbox_st->spinbox);
...
{
    lv_coord_t h = _gui_get_spinbox_height(spinbox_st->spinbox);
    spinbox_st->minus_button = lv_btn_create(parent);
    lv_obj_set_size(spinbox_st->minus_button, h, h);
    lv_obj_align_to(spinbox_st->minus_button, spinbox_st->spinbox, LV_ALIGN_OUT_LEFT_MID, -5, 0);
    lv_obj_set_style_bg_img_src(spinbox_st->minus_button, LV_SYMBOL_MINUS, 0);
    lv_obj_add_event_cb(spinbox_st->minus_button, lv_spinbox_decrement_event_cb, LV_EVENT_ALL, spinbox_st->spinbox);
...
{
    spinbox_st->spinbox = lv_spinbox_create(parent);
    ...
    lv_obj_align(spinbox_st->spinbox, LV_ALIGN_CENTER, spinbox_st->x_ofs, spinbox_st->y_ofs);
}
